...
class BluetoothHidTransmitter(private val context: Context, private val onStatusChange: (String, Boolean) -> Unit) : OutputTransmitter {
    ...
    // THE FIX: 3-in-1 Combo Descriptor (Keyboard, Gamepad, Mouse)
    private val HID_REPORT_DESC = byteArrayOf(
        ...
        // --- GAMEPAD (Report ID 2) ---
        0x05, 0x01, 0x09, 0x05, 0xA1.toByte(), 0x01, 0x85.toByte(), 0x02,
        ...
        // --- MOUSE (Report ID 3) ---
        0x05, 0x01, 0x09, 0x02, 0xA1.toByte(), 0x01, 0x85.toByte(), 0x03,
            ...
            override fun onServiceConnected(profile: Int, proxy: BluetoothProfile) {
                if (profile == BluetoothProfile.HID_DEVICE) {
                    ...
                    val sdp = BluetoothHidDeviceAppSdpSettings(
                        "JoyType Handheld", "Universal Input Engine", "Vazbloke",
                        0xC0.toByte(), // Ensure this is 0xC0 (Combo Device)
                        HID_REPORT_DESC
                    )
    ...
    override fun sendMouseState(leftClick: Boolean, rightClick: Boolean, dx: Int, dy: Int) {
        ...
        hidDevice?.sendReport(connectedHost, 3, report) // Blast instantly on ID 3!
    }
}